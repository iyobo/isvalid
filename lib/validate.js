...
var ValidationError = require('./errors/validation.js'),
	testRange = require('./tools/test-range.js'),
	unique = require('./tools/unique.js'),
	allowIO = require('./tools/allow-io.js'),
	formalize = require('./formalize.js');

function validateObject(data, schema, fn, keyPath, options) {
...
function validateArray(data, schema, fn, keyPath, options) {
	...
	if (data) {
		...
		var validateNext = function(idx) {
			...
			if (idx === data.length) {
				...
				if (schema.len && !testRange(schema.len, validArray.length)) {
					return fn(
						new ValidationError(
							keyPath,
							schema._nonFormalizedSchema,
...
function validateString(str, schema, fn, keyPath, options) {
...
function validateNumber(num, schema, fn, keyPath, options) {
	...
	if (schema.range) {
		if (!testRange(schema.range, validNum)) {
			return fn(
				new ValidationError(
					keyPath,
					schema._nonFormalizedSchema,
...
function validateBoolean(val, schema, fn, keyPath, options) {
...
function validateDate(val, schema, fn, keyPath, options) {
...
function validateCustom(data, schema, fn, keyPath, options) {

	var finalize = function(data, fn) {
		if (fn) return allowIO(fn, null, data);
	};
...
function validateAny(data, schema, fn, keyPath, options) {

	// If schema is not yet formalized - formalize it and come back.
	if (schema._nonFormalizedSchema === undefined) {
		return formalize(schema, function(err, formalizedSchema) {
			if (err) return fn(err);
	...
	if ('Object' === schema.type) return validateObject(data, schema, fn, keyPath, options);
	if ('Array' === schema.type) return validateArray(data, schema, fn, keyPath, options);
	if ('String' === schema.type) return validateString(data, schema, fn, keyPath, options);
	if ('Number' === schema.type) return validateNumber(data, schema, fn, keyPath, options);
	if ('Boolean' === schema.type) return validateBoolean(data, schema, fn, keyPath, options);
	if ('Date' === schema.type) return validateDate(data, schema, fn, keyPath, options);

	// This error should have been eliminated by the formalizer.
	throw new Error('Cannot validate schema of type ' + schema.type + '.');

}

module.exports = exports = function(data, schema, fn, keyPath, options) {
