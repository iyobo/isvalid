...
function finalize(formalizedSchema, nonFormalizedSchema, fn) {
	...
	else tools.allowIO(fn, null, formalizedSchema);
...
function formalizeObject(formalizedSchema, nonFormalizedSchema, fn) {
	...
	(function formalizeNextKey(idx) {
		...
		formalizeAny(formalizedSchema.schema[key], tools.sync(fn.sync, function(err, formalizedKey) {
			...
			else tools.allowIO(formalizeNextKey, idx + 1);
...
function formalizeArray(formalizedSchema, nonFormalizedSchema, fn) {
	...
	return formalizeAny(formalizedSchema.schema, tools.sync(fn.sync, function(err, formalizedSubschema) {
...
function formalizeAny(schema, fn) {
	...
	fn = fn || tools.sync(true, function(err, s) {
		if (err) throw err;
		return s;
	});
