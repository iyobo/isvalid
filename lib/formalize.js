...
function finalize(formalizedSchema, nonFormalizedSchema, fn, sync) {
	...
	if (fn) {
		...
		else allowIO(fn, formalizedSchema);
	}

}

function formalizeObject(formalizedSchema, nonFormalizedSchema, fn, sync) {
...
function formalizeArray(formalizedSchema, nonFormalizedSchema, fn, sync) {
...
function formalizeAny(schema, fn, sync) {
	...
	for (var key in schema) {
		...
		if (formalizedSchema !== null && ['Object', 'Array', 'String', 'Number', 'Boolean'].indexOf(formalizedSchema[key].constructor.name) == -1) {
			formalizedSchema[key].toJSON = formalizedSchema[key].toString;
		}
